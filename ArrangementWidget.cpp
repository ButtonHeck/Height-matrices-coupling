...
void ArrangementWidget::updateProfilesData(const HeightMatrix &masterMatrix, HeightMatrix &targetMatrix, int masterSide, int targetSide)
{
...
void ArrangementWidget::updateArrangedProfile(const HeightMatrix &mMatrix, HeightMatrix &tMatrix, int mSide)
{
  ...
  if (mSide == LEFT || mSide == RIGHT)
    {
      HeightMatrix::ConstColumnIterator mColumn = (mSide == LEFT) ? mMatrix.columnBegin(0) : mMatrix.columnBegin(mMatrix.getWidth() - 1);
  ...
  //comparing TOP side of master matrix with BOTTOM side of target matrix or vice versa
    ...
    {
      HeightMatrix::ConstRowIterator mRow = (mSide == TOP) ? mMatrix.rowBegin(0) : mMatrix.rowBegin(mMatrix.getHeight() - 1);
...
void ArrangementWidget::updateTargetMatrix(HeightMatrix& matrix, int side)
{
  if (side == LEFT || side == RIGHT)
    {
      HeightMatrix::ColumnIterator column = (side == LEFT) ? matrix.columnBegin(0) : matrix.columnBegin(matrix.getWidth() - 1);
    ...
    {
      HeightMatrix::RowIterator row = (side == TOP) ? matrix.rowBegin(0) : matrix.rowBegin(matrix.getHeight() - 1);
      //all odd indices for each pair in profile vector represents point height, thus +1 is applied
      for (; row.isValid(); row++)
